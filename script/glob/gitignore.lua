...
local function object(type, pat)
    return m.Ct(m.Cg(m.Cc(type), 'type') * m.Cg(pat, 'value'))
end
...
local parser = m.P({
    ...
    ['Main'] = m.Ct(
        m.V('Sp')
            * m.P('{')
            * m.V('Pattern')
            * (',' * expect(m.V('Pattern'), 'Miss exp after ","')) ^ 0
            * m.P('}')
    ) + m.Ct(m.V('Pattern')) + m.T('Main Failed'),
    ['Pattern'] = m.Ct(
        m.V('Sp') * prop('neg', m.P('!')) * expect(m.V('Unit'), 'Miss exp after "!"')
    ) + m.Ct(m.V('Unit')),
    ['NeedRoot'] = prop('root', (m.P('.') * m.V('Slash') + m.V('Slash'))),
    ['Unit'] = m.V('Sp') * m.V('NeedRoot') ^ -1 * expect(m.V('Exp'), 'Miss exp') * m.V('Sp'),
    ['Exp'] = m.V('Sp') * (m.V('FSymbol') + object('/', m.V('Slash')) + m.V('Word')) ^ 0 * m.V(
        'Sp'
    ),
    ['Word'] = object('word', m.Ct((m.V('CSymbol') + m.V('Char') - m.V('FSymbol')) ^ 1)),
    ['CSymbol'] = object('*', m.P('*')) + object('?', m.P('?')) + object('[]', m.V('Range')),
