...
local function getValidChanges(uri, changes)
  ...
  for _, change in ipairs(changes) do
    if scp:isChildUri(change.uri) or scp:isLinkedUri(change.uri) then
...
local function tryModifyClient(uri, finalChanges)
  ...
  for _, change in ipairs(finalChanges) do
    if change.uri and (scp:isChildUri(change.uri) or scp:isLinkedUri(change.uri)) then
...
local function buildMaunuallyMessage(changes)
  ...
  for _, change in ipairs(changes) do
    if change.action == 'add' then
      message[#message + 1] = '* '
        .. lang.script('WINDOW_MANUAL_CONFIG_ADD', change.key, change.value)
    elseif change.action == 'set' then
      message[#message + 1] = '* '
        .. lang.script('WINDOW_MANUAL_CONFIG_SET', change.key, change.value)
    elseif change.action == 'prop' then
      message[#message + 1] = '* '
        .. lang.script('WINDOW_MANUAL_CONFIG_PROP', change.key, change.prop, change.value)
    end
  end
...
function m.editText(uri, edits)
  ...
  for i, edit in ipairs(edits) do
    textEdits[i] =
      converter.textEdit(converter.packRange(state, edit.start, edit.finish), edit.text)
  end
  local params = {
    edit = {
      changes = {
...
function m.editMultiText(editors)
  ...
  for _, editor in ipairs(editors) do
    ...
    if state then
      ...
      local edit =
        converter.textEdit(converter.packRange(state, editor.start, editor.finish), editor.text)
