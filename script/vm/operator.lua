...
--- @param operators parser.object.base[]
--- @param op string
...
function vm.runOperator(op, exp, value)
    ...
    for c in node:eachObject() do
        if c.type == 'string' or c.type == 'doc.type.string' then
...
vm.binarySwitch = util.switch()
    ...
    :call(function(source)
        ...
        local op = source.op.type
        local zero = b == 0 and (op == '%' or op == '/' or op == '//')
        if a and b and not zero then
            ...
            if op == '+' or op == '-' or op == '*' or op == '%' then
                ...
                if infer1:hasType(uri, 'integer') and infer2:hasType(uri, 'integer') then
                    vm.setNode(source, vm.declareGlobal('type', 'integer'))
                    return
                end
                if
                    (infer1:hasType(uri, 'number') or infer1:hasType(uri, 'integer'))
                    and (infer2:hasType(uri, 'number') or infer2:hasType(uri, 'integer'))
                then
                    vm.setNode(source, vm.declareGlobal('type', 'number'))
                    return
                end
            ...
            if op == '/' or op == '^' then
                ...
                local infer2 = vm.getInfer(source[2])
                if
                    (infer1:hasType(uri, 'integer') or infer1:hasType(uri, 'number'))
                    and (infer2:hasType(uri, 'integer') or infer2:hasType(uri, 'number'))
                then
                    vm.setNode(source, vm.declareGlobal('type', 'number'))
                    return
                end
    ...
    :case('..')
    :call(function(source)
        local a = vm.getString(source[1]) or vm.getNumber(source[1])
        local b = vm.getString(source[2]) or vm.getNumber(source[2])
        if a and b then
