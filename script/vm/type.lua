...
function vm.isSubType(uri, child, parent, mark, errs)
    ...
    if config.get(uri, 'Lua.type.weakUnionCheck') then
      ...
      for n in child:eachObject() do
        ...
        if
          ...
          and vm.isSubType(uri, n, parent, mark, errs) == false
        then
          if errs then
        ...
        if set.type == 'doc.class' and set.extends then
          for _, ext in ipairs(set.extends) do
            if
              ...
              and vm.isSubType(uri, ext[1], parent, mark, errs) == true
            then
              mark[childName] = nil
              return true
            end
  ...
  if
    ...
    and vm.isSubType(uri, parentName, childName, mark)
  then
    return true
  end
    ...
    for _, paramName in ipairs(params) do
      ...
      elseif value.type == 'variable' then
        ...
        if paramName == 'key' then
          ...
          lparams[paramName] = vm.getInfer(value):view(uri) or vm.getInfer(value):view(uri)
        end
      end
      index = index + 1
    end
