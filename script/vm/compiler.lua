    ...
    for _, set in ipairs(sets) do
      if set.type == 'doc.class' then
        ...
        if not searchedFields[key] and set.bindSource then
          ...
          if src.value and src.value.type == 'select' and src.value.vararg.type == 'call' then
            local func = src.value.vararg.node
            local args = src.value.vararg.args
            if func.special == 'setmetatable' and args and args[1] and args[1].type == 'table' then
        ...
        if not searchedFields[key] and set.bindSource then
          ...
          searchFieldSwitch(src.type, suri, src, key, function(field)
            ...
            if fieldKey and not searchedFields[fieldKey] then
              if not searchedFields[fieldKey] and guide.isAssign(field) and field.value then
                if
