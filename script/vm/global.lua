...
local compilerGlobalSwitch = util
  ...
  :case('call')
  :call(function(source)
    if source.node.special == 'rawset' or source.node.special == 'rawget' then
...
function vm.getGlobalFields(cate, name)
  ...
  for gid, global in pairs(allGlobals) do
    if
      ...
      and not gid:find(vm.ID_SPLITE, #key + 2)
    then
...
function vm.getGlobals(cate)
  ...
  local clock = os.clock()
  for gid, global in pairs(allGlobals) do
    if util.stringStartWith(gid, cate) and not gid:find(vm.ID_SPLITE) then
      globals[#globals + 1] = global
    end
  end
  local cost = os.clock() - clock
