...
function Tracer:collectCare(obj)
    while true do
        ...
        if self.fastCalc then
            if obj.type == 'if' or obj.type == 'while' or obj.type == 'binary' then
                self.fastCalc = false
            end
            if obj.type == 'call' and obj.node then
                if obj.node.special == 'assert' or obj.node.special == 'type' then
                    self.fastCalc = false
                end
            end
        end
...
function Tracer:collectLocal()
    ...
    if variable.base.type ~= 'local' and variable.base.type ~= 'self' then
    ...
    for _, cast in ipairs(casts) do
        if
            cast.name[1] == self.name
            and cast.start > startPos
            and cast.finish < finishPos
            and vm.getCastTargetHead(cast) == variable.base
        then
...
local lookIntoChild = util.switch()
    ...
    :call(
        ...
        function(tracer, action, topNode, outNode)
            ...
            if action.filter then
                blockNode, mainNode =
                    tracer:lookIntoChild(action.filter, topNode:copy(), topNode:copy())
            else
    ...
    :call(
        ...
        function(tracer, action, topNode, outNode)
            ...
            if action.value and action.ref and action.value.type == 'select' then
                ...
                then
                    ...
                    if getVar and tracer.getMap[getVar] then
    ...
    :call(
        ...
        function(tracer, action, topNode, outNode)
            ...
            elseif action.op.type == 'or' then
                ...
                local topNode2, outNode2 =
                    tracer:lookIntoChild(action[2], outNode1, outNode1:copy())
