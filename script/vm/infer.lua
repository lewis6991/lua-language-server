...
function mt:hasUnknown(uri)
  self:_computeViews(uri)
  return not next(self.views) or self.views['unknown'] == true
end
...
function mt:hasFunction(uri)
  self:_computeViews(uri)
  return self.views['function'] == true or self._hasDocFunction == true
end
...
function mt:view(uri, default)
  if self._lastView and self._lastViewUri == uri and self._lastViewDefault == default then
  ...
  if #array == 0 then
    ...
    if max > limit then
      view = string.format('%s...(+%d)', table.concat(array, '|', 1, limit), max - limit)
    else
  ...
  if self.node:isOptional() then
    if #array == 0 then
      ...
      if max > 1 or view:find(guide.notNamePattern .. guide.namePattern .. '$') then
        view = '(' .. view .. ')?'
      else
        view = view .. '?'
      end
...
function mt:viewLiterals()
  ...
  local literals = {}
  for n in self.node:eachObject() do
    if n.type == 'string' or n.type == 'number' or n.type == 'integer' or n.type == 'boolean' then
      local literal
