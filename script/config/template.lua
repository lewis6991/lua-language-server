...
register('Hash', {}, function(self, value)
    if type(value) == 'table' then
        if #value == 0 then
            for k, v in pairs(value) do
                if not self.subkey:checker(k) or not self.subvalue:checker(v) then
                    return false
                end
            end
        else
    ...
    if type(value) == 'string' then
        return self.subkey:checker('') and self.subvalue:checker(true)
    end
end, function(self, value)
    if type(value) == 'table' then
        local t = {}
...
local template = {
    ...
    ['Lua.runtime.special'] = Type.Hash(Type.String, Type.String >> 'require' << {
        '_G',
        'rawset',
        'rawget',
        'setmetatable',
    ...
    ['Lua.runtime.pluginArgs'] = Type.Or(
    ...
    ['Lua.diagnostics.disable'] = Type.Array(
        Type.String << util.getTableKeys(diag.getDiagAndErrNameMap(), true)
    ),
    ...
    ['Lua.nameStyle.config'] = Type.Hash(
        Type.String,
        Type.Or(Type.String, Type.Array(Type.Hash(Type.String, Type.String)))
    ) >> {},
