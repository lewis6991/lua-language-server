...
function mt:getRequireNameByPath(path, searcher)
  ...
  local stemSearcher = searcher:gsub('%.[^%.]+$', ''):gsub('[/\\]+', separator)
...
function mt:getRequireResultByPath(path)
  ...
  for _, searcher in ipairs(searchers) do
    local isAbsolute = searcher:match('^[/\\]') or searcher:match('^%a+%:')
    ...
    if parentCount > 0 then
      local parentPath = libraryPath or (self.scp.uri and furi.decode(self.scp.uri))
      if parentPath then
        local tail
  ...
  for _, searcher in ipairs(searchers) do
    ...
    for uri in files.eachFile(self.scp.uri) do
      if
        ...
        and (not vm.isMetaFile(uri) or vm.isMetaFileRequireable(uri))
      then
        ...
        if not strict or relative == '/' or relative == '' then
          results[#results + 1] = uri
