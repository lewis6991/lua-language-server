...
function m.getTargetSource(state)
    ...
    if
        targetSource.type ~= 'getlocal'
        and targetSource.type ~= 'table'
        and targetSource.type ~= 'function'
    then
        return nil
    end
    return targetSource
    ...
    for uri in files.eachFile(state.uri) do
        ...
        for _, sr in ipairs(infos) do
            ...
            if sr.searcher == '[[meta]]' then
                ...
                local pattern = sr.searcher:gsub('(%p)', '%%%1'):gsub('%%%?', '(.-)')
            ...
            then
                ...
                if targetSource.type == 'getlocal' and vm.getDeprecated(targetSource.node) then
                    goto INNER_CONTINUE
                end
