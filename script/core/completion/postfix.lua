...
local function hasNonFieldInNode(source)
    ...
    while source ~= block do
        if source.type == 'call' or source.type == 'getindex' or source.type == 'getmethod' then
            return true
        end
...
register('function')({
    function(state, source, callback)
        ...
        callback(
            string.format('function %s($1)\n\t$0\nend', subber(source.start + 1, source.finish))
        )
    end,
})
    ...
    function(state, source, callback)
        ...
        if source.type == 'call' then
            if source.args and #source.args > 0 then
                ...
                callback(
                    string.format('pcall(%s)', subber(source.node.start + 1, source.node.finish))
                )
            end
        else
            callback(string.format('pcall(%s$1)$0', subber(source.start + 1, source.finish)))
        end
    end,
})
    ...
    function(state, source, callback)
        ...
        if source.type == 'call' then
            if source.args and #source.args > 0 then
                callback(
                    string.format(
                        'xpcall(%s, ${1:debug.traceback}, %s)$0',
                        subber(source.node.start + 1, source.node.finish),
                        subber(source.args[1].start + 1, source.args[#source.args].finish)
                    )
                )
            ...
            callback(
                string.format(
                    'xpcall(%s, ${1:debug.traceback}$2)$0',
...
register('local')({
    function(state, source, callback)
        ...
        local subber = subString(state)
        callback(string.format('local $1 = %s$0', subber(source.start + 1, source.finish)))
    end,
})
...
register('unpack')({
    function(state, source, callback)
        ...
        local subber = subString(state)
        callback(string.format('unpack(%s)', subber(source.start + 1, source.finish)))
    end,
})
...
register('insert')({
    function(state, source, callback)
        ...
        local subber = subString(state)
        callback(string.format('table.insert(%s, $0)', subber(source.start + 1, source.finish)))
    end,
})
...
register('remove')({
    function(state, source, callback)
        ...
        local subber = subString(state)
        callback(string.format('table.remove(%s, $0)', subber(source.start + 1, source.finish)))
    end,
})
...
register('concat')({
    function(state, source, callback)
        ...
        local subber = subString(state)
        callback(string.format('table.concat(%s, $0)', subber(source.start + 1, source.finish)))
    end,
})
...
local function checkPostFix(state, word, wordPosition, position, symbol, results)
    local source = guide.eachSourceContain(state.ast, wordPosition, function(source)
        if accepts[source.type] and source.finish == wordPosition then
            return source
        end
    end)
    if not source then
