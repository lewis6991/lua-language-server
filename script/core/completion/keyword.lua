...
\t$0\
    ...
    function(info)
      return guide.eachSourceContain(info.state.ast, info.start, function(source)
        if source.type == 'while' or source.type == 'in' or source.type == 'loop' then
  ...
  { 'else' },
  {
    ...
    function(info, results)
      ...
      if info.text:find('^%s*then', offset + 1) or info.text:find('^%s*do', offset + 1) then
        return false
      end
      if info.hasSpace then
        results[#results + 1] = {
...
until $1',
  ...
  },
  {
    ...
    function(info, results)
      ...
      if first == 'end' or first == 'else' or first == 'elseif' then
...
\t$0\
  ...
  },
  {
    ...
    function(info, results)
      ...
      local tabStr =
        info.state.lua:sub(info.state.lines[endLine], guide.positionToOffset(info.state, endPos))
      local newText
      ...
      local word =
        lookback.findWord(info.state.lua, guide.positionToOffset(info.state, info.start) - 1)
      if word ~= 'goto' then
