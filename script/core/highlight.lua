...
local function find(source, callback)
  ...
  elseif source.type == 'field' or source.type == 'method' then
    eachRef(source.parent, callback)
  elseif source.type == 'getindex' or source.type == 'setindex' or source.type == 'tableindex' then
    eachRef(source, callback)
  elseif source.type == 'setglobal' or source.type == 'getglobal' then
    eachRef(source, callback)
  elseif source.type == 'goto' or source.type == 'label' then
    eachRef(source, callback)
  elseif source.type == 'string' and source.parent and source.parent.index == source then
    eachRef(source.parent, callback)
  elseif
    source.type == 'string'
...
local function checkInIf(state, source, text, position)
  ...
  if
    position >= source.finish - #'end'
    and position <= source.finish
    and text
    and text:sub(endA, endB) == 'end'
  then
    return true
  end
  -- 检查每个子模块
...
local function isRegion(str)
  if str:sub(1, #'region') == 'region' or str:sub(1, #'#region') == '#region' then
    return true
  end
  return false
end
...
local function isEndRegion(str)
  if str:sub(1, #'endregion') == 'endregion' or str:sub(1, #'#endregion') == '#endregion' then
    return true
  end
  return false
end
...
local function checkRegion(ast, offset, callback)
  ...
  for i, comment in ipairs(ast.comms) do
    if comment.type == 'comment.short' then
      if comment.start <= offset and comment.finish >= offset then
...
return function(uri, offset)
  ...
  if source then
    ...
    find(source, function(target)
      ...
      elseif target.type == 'setfield' or target.type == 'tablefield' then
        target = target.field
      ...
      elseif target.type == 'setindex' or target.type == 'tableindex' then
        target = target.index
        kind = define.DocumentHighlightKind.Write
      elseif target.type == 'getlocal' or target.type == 'getglobal' or target.type == 'goto' then
        kind = define.DocumentHighlightKind.Read
      elseif
        target.type == 'setlocal'
        or target.type == 'local'
        or target.type == 'setglobal'
        or target.type == 'label'
      then
        kind = define.DocumentHighlightKind.Write
      elseif
        target.type == 'string'
