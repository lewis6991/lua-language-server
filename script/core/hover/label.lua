    ...
    for _, doc in ipairs(defs) do
        ...
        if doc.type == 'doc.alias' then
            return '(alias) '
                .. doc.alias[1]
                .. ' '
                .. lang.script('HOVER_EXTENDS', vm.getInfer(doc.extends):view(guide.getUri(source)))
        end
...
local function asValue(source, title)
    ...
    if cont and (type == 'table' or type == 'any' or type == 'unknown' or type == 'nil') then
    else
        pack[#pack + 1] = type
    end
    if literal then
...
local function isGlobalField(source)
    ...
    elseif source.type == 'tablefield' then
        ...
        if parent.type == 'setglobal' or parent.type == 'getglobal' then
            return true
        end
...
return function(source, oop)
    ...
    elseif source.type == 'doc.type.name' or source.type == 'doc.enum.name' then
