...
local function asField(source, oop)
  ...
  local node = class or buildName(source.node, false) or guide.getKeyName(source.node) or '?'
...
local function asDocFunction(source, oop)
  local doc = guide.getParentType(source, 'doc.type') or guide.getParentType(source, 'doc.overload')
...
function buildName(source, oop)
  ...
  if source.type == 'setfield' or source.type == 'getfield' then
    return asField(source, oop) or '', oop
  end
