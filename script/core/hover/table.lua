...
local function buildAsHash(uri, keys, nodeMap, reachMax)
  ...
  for _, key in ipairs(keys) do
    ...
    if literalView then
      ...
      lines[#lines + 1] = ('    %s%s: %s,'):format(key, isOptional and '?' or '', typeView)
    end
  end
  if reachMax > 0 then
...
local function getVisibleKeyMap(source, fields)
  ...
  for _, field in ipairs(fields) do
    ...
    if not ignored[key] and vm.isVisible(source, field) then
...
return function(source)
  ...
  for n in node:eachObject() do
    if n.type == 'global' and n.cate == 'type' then
      if
        n.name == 'string'
        or (n.name ~= 'unknown' and n.name ~= 'any' and vm.isSubType(uri, n.name, 'string'))
      then
        return nil
      end
    elseif n.type == 'doc.type.string' or n.type == 'string' then
      return nil
