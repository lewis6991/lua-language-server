...
local function findNearCall(uri, ast, pos)
  ...
  guide.eachSourceContain(ast.ast, pos, function(src)
    if src.type == 'call' or src.type == 'table' or src.type == 'function' then
      local finishOffset = guide.positionToOffset(state, src.finish)
      -- call(),$
      if src.finish <= pos and text:sub(finishOffset, finishOffset) == ')' then
        return
      end
      -- {},$
      if src.finish <= pos and text:sub(finishOffset, finishOffset) == '}' then
...
local function isEventNotMatch(call, src)
  ...
  if not event.extends or #event.extends.types ~= 1 then
    return false
  end
...
local function makeSignatures(text, call, pos)
  ...
  if call.args then
    ...
    if not index then
      ...
      if backSymbol == ',' or backSymbol == '(' then
        index = #args + 1
      else
        index = #args
      end
  ...
  for src in node:eachObject() do
    if
      (src.type == 'function' and not vm.isVarargFunctionWithOverloads(src))
      or src.type == 'doc.type.function'
    then
      if not mark[src] and not isEventNotMatch(call, src) then
        mark[src] = true
      ...
      for _, set in ipairs(src:getSets(guide.getUri(call))) do
        if set.type == 'doc.class' then
          for _, overload in ipairs(set.calls) do
            ...
            if not mark[f] and not isEventNotMatch(call, src) then
              mark[f] = true
