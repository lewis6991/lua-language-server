...
function mt:resolveReference(source)
  ...
  if client.getOption('codeLensViewReferences') then
    ...
    for _, ref in ipairs(refs or {}) do
      ...
      if state then
        locations[#locations + 1] =
          conv.location(ref.uri, conv.packRange(state, ref.target.start, ref.target.finish))
      end
    end
    local command =
      conv.command(lang.script('COMMAND_REFERENCE_COUNT', count), 'lua.showReferences', {
        self.uri,
    ...
    local command = conv.command(lang.script('COMMAND_REFERENCE_COUNT', count), '', {})
    return command
  end
end
