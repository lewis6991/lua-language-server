...
local function buildName(source, sub)
    ...
    if source.type == 'setfield' or source.type == 'tablefield' or source.type == 'getfield' then
...
local function buildTable(tbl, sub)
    ...
    for i = 1, 5 do
        ...
        elseif field.type == 'tableindex' and field.index then
...
local function buildValue(source, sub, used, symbols)
    ...
    if source.value then
        ...
        elseif source.value.type == 'number' or source.value.type == 'integer' then
        ...
        elseif source.value.type == 'table' then
            ...
            if #source.value > 0 then
                if lastField.type == 'tableexp' and lastField.tindex == #source.value then
                    -- Array
...
local function buildAnonymous(source, sub, used, symbols)
    ...
    elseif source.type == 'table' then
        ...
        if lastField then
            if lastField.type == 'tableexp' and lastField.tindex == #source then
                -- Array
                kind = define.SymbolKind.Array
                details[#details + 1] = '['
...
local function buildBlock(source, sub, used, symbols)
    ...
    elseif source.type == 'loop' or source.type == 'in' then
        symbols[#symbols + 1] = {
            name = 'for',
...
local function buildSource(source, sub, used, symbols)
    ...
    elseif source.type == 'function' or source.type == 'table' then
...
local function packChild(symbols)
    ...
    local function insertChilds(min, max)
        ...
        while true do
            ...
            if symbol.selectionRange[1] < min or symbol.selectionRange[2] > max then
                break
            end
            if not list then
                list = {}
