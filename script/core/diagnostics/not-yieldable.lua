  ...
  for _, def in ipairs(defs) do
    if def.type == 'function' then
      ...
      if arg then
        hasFuncDef = true
        if
          vm.getInfer(arg):hasType(guide.getUri(def), 'any')
    ...
    if def.type == 'doc.type.function' then
      ...
      if arg then
        hasFuncDef = true
        if
          vm.getInfer(arg.extends):hasType(guide.getUri(def), 'any')
          or vm.isAsync(arg.extends, true)
        then
          return true
        end
      end
