...
return function(uri, callback)
    ...
    guide.eachSourceType(state.ast, 'table', function(src)
        ...
        for _, def in ipairs(defs) do
            ...
            if def.type == 'doc.type.array' or def.type == 'doc.type.table' then
                return
            end
        end
        ...
        for className, samedefs in pairs(sortedDefs) do
            ...
            for _, def in ipairs(samedefs) do
                ...
                for _, field in ipairs(def.fields) do
                    if not field.optional and not vm.compileNode(field):isNullable() then
            ...
            warnings[#warnings + 1] =
                lang.script('DIAG_MISSING_FIELDS', className, table.concat(missedKeys, ', '))
        end
