...
local function allLiteral(source)
    ...
    guide.eachSource(source, function(src)
        if src.type ~= 'unary' and src.type ~= 'binary' and not guide.isLiteral(src) then
            result = false
            return false
        end
    end)
...
local function hasReturn(block)
    ...
    if block.type == 'if' then
        ...
        if block.type == 'while' then
            if vm.testCondition(block.filter) and not block.breaks and allLiteral(block.filter) then
                return true
            end
        end
