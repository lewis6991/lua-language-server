...
local function checkRequire(source)
    ...
    elseif libName == 'dofile' or libName == 'loadfile' then
...
local function processDef(source, src)
    ...
    if src.type == 'getindex' or src.type == 'setindex' or src.type == 'tableindex' then
        src = src.index
        if not src then
            return
        end
...
return function(uri, offset)
    ...
    local results = {}
    for _, uri0 in ipairs(checkRequire(source) or {}) do
        results[#results + 1] = {
            uri = uri0,
            source = source,
            target = {
    ...
    for _, src in ipairs(defs) do
        results[#results + 1] = processDef(source, src)
    end
