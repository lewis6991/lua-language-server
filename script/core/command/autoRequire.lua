...
local function findInsertRow(uri)
  ...
  for i = 0, #lines do
    ...
    if not lnText:find('require', 1, true) then
      ...
      if
        not lnText:match('^local%s')
        and not lnText:match('^%s*$')
        and not lnText:match('^%-%-')
      then
        break
      end
    else
...
local function askAutoRequire(uri, visiblePaths)
  ...
  local result =
    client.awaitRequestMessage('Info', lang.script.COMPLETION_ASK_AUTO_REQUIRE, selects)
  if not result then
    return
  end
...
local function applyAutoRequire(uri, row, name, result, fmt)
  ...
  if fmt.quot == "'" then
    quotedResult = ([['%s']]):format(
      quotedResult:sub(2, -2):gsub([[']], [[\']]):gsub([[\"]], [["]])
    )
  end
  if fmt.pair then
