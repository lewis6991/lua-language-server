  ...
  while true do
    ...
    then
      ...
      if not decode_escape_set[nx] then
        statusPos = i
        decode_error(
          "invalid escape char '" .. (nx and string_char(nx) or '<eol>') .. "' in string"
        )
      end
      has_escape = true
      i = i + 2
...
local function query_(ast, pathlst, n)
  ...
  if type(data) ~= 'table' then
    return nil,
      string_format(
        'path `%s` does not point to object or array',
        '/' .. table_concat(pathlst, '/', 1, n - 1)
      )
  end
  local k = pathlst[n]
  ...
  if isarray then
    if k == '-' then
      ...
      if k:match('^0%d+') then
        return nil,
          string_format(
            'path `%s` point to array, but invalid',
            '/' .. table_concat(pathlst, '/', 1, n)
          )
      end
      k = tonumber(k)
