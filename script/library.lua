...
local function initBuiltIn(uri)
  ...
  ---@type fs.path
  local metaPath = fs.path(METAPATH)
    / config.get(uri, 'Lua.runtime.meta'):gsub('%$%{(.-)%}', {
      version = version,
      language = langID,
      encoding = encoding,
    })
...
local function askFor3rd(uri, cfg, checkThirdParty)
  ...
  elseif checkThirdParty == 'Ask' then
    ...
    local result = client.awaitRequestMessage(
      'Info',
    ...
    for _, cfg in ipairs(configs) do
      ...
      for _, word in ipairs(cfg.words) do
        ...
        if wholeMatch(text, word) then
          log.info(
            'Found 3rd library by word: ',
            word,
            uri,
            library,
            inspect(config.get(uri, 'Lua.workspace.library'))
          )
          ---@async
          await.call(function()
