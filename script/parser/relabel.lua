...
local exp = m.P({
    ...
    Suffix = m.Cf(
        m.V('Primary')
            * (S * (m.P('+') * m.Cc(1, mt.__pow) + m.P('*') * m.Cc(0, mt.__pow) + m.P('?') * m.Cc(
                    ...
                    m.Cg(num * m.Cc(mult))
                        + m.Cg(m.C(m.S('+-') * m.R('09') ^ 1) * m.Cc(mt.__pow) + name * m.Cc('lab')),
                    'ExpNumName'
                ) + '->' * expect(
                    S
                        * (
                    ...
                    'ExpCap'
                ) + '=>' * expect(S * m.Cg(Def / getdef * m.Cc(m.Cmt)), 'ExpName1')))
                ^ 0,
        function(a, b, f)
            if f == 'lab' then
                return a + mm.T(b)
    ...
    Primary = '(' * expect(m.V('Exp'), 'ExpPatt4') * expect(S * ')', 'MisClose1')
        ...
        + Class
        + defined
        + '%' * expect(m.P('{'), 'ExpNameOrLab') * expect(S * m.V('Label'), 'ExpLab1') * expect(
            S * '}',
            'MisClose7'
        ) / mm.T
        ...
        + '{' * expect(m.V('Exp'), 'ExpPattOrClose') * expect(S * '}', 'MisClose5') / mm.C
        + m.P('.') * m.Cc(any)
        + (name * -arrow + '<' * expect(name, 'ExpName3') * expect('>', 'MisClose6'))
            * m.Cb('G')
            / NT,
    Label = num + name,
    Definition = name * arrow * expect(m.V('Exp'), 'ExpPatt8'),
    Grammar = m.Cg(m.Cc(true), 'G')
        * m.Cf(m.V('Definition') / firstdef * (S * m.Cg(m.V('Definition'))) ^ 0, adddef)
        / mm.P,
})
