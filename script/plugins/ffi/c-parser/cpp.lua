...
eval_exp = typed('Ctx, Exp -> number', function(ctx, exp)
  ...
  elseif exp.op == '/' then
    return eval_exp(ctx, exp[1]) / eval_exp(ctx, exp[2])
  elseif exp.op == '>>' then
    return shr(eval_exp(ctx, exp[1]), eval_exp(ctx, exp[2])) -- FIXME C semantics
  elseif exp.op == '<<' then
    return shl(eval_exp(ctx, exp[1]), eval_exp(ctx, exp[2])) -- FIXME C semantics
  elseif exp.op == '==' then
    return (eval_exp(ctx, exp[1]) == eval_exp(ctx, exp[2])) and 1 or 0
  elseif exp.op == '!=' then
    return (eval_exp(ctx, exp[1]) ~= eval_exp(ctx, exp[2])) and 1 or 0
  elseif exp.op == '>=' then
    return (eval_exp(ctx, exp[1]) >= eval_exp(ctx, exp[2])) and 1 or 0
  ...
  elseif exp.op == '<' then
    return (eval_exp(ctx, exp[1]) < eval_exp(ctx, exp[2])) and 1 or 0
  elseif exp.op == '!' then
    return (eval_exp(ctx, exp[1]) == 1) and 0 or 1
  elseif exp.op == '&&' then
...
local consume_parentheses = typed(
  '{string}, number, LineList, number -> {{string}}, number',
...
local replace_args = typed(
  'Ctx, {string}, table, LineList, number -> ()',
...
macro_expand = typed(
  'Ctx, {string}, LineList, number, boolean -> ()',
...
cpp.parse_file = typed('string, FILE*?, Ctx? -> Ctx?, string?', function(filename, fd, ctx)
  ...
  for cur, lineitem in ipairs(linelist) do
    ...
    if
      #ifmode == 1 and (tk.directive == 'elif' or tk.directive == 'else' or tk.directive == 'endif')
    then
    ...
    elseif ifmode[#ifmode] == false then
      if tk.directive == 'ifdef' or tk.directive == 'ifndef' or tk.directive == 'if' then
    ...
    elseif ifmode[#ifmode] == 'skip' then
      if tk.directive == 'ifdef' or tk.directive == 'ifndef' or tk.directive == 'if' then
        table.insert(ifmode, 'skip')
      elseif tk.directive == 'else' or tk.directive == 'elif' then
        -- do nothing
      elseif tk.directive == 'endif' then
        table.remove(ifmode, #ifmode)
      end
