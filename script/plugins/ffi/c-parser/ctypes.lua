...
local get_composite_type = typed(
    'TypeList, string?, string, array, string, function -> CType, string',
    function(lst, specid, spectype, parts, partsfield, get_parts)
        local name = specid
        local key = spectype
            .. '@'
            .. (name or ctypes.TESTMODE and 'anonymous' or getAnonymousID(parts))
        ...
        if lst[key] then
            if
                typ[partsfield]
                and lst[key][partsfield]
                and not equal_declarations(typ, lst[key])
            then
...
local function get_enum(lst, spec)
    ...
    local typ, key =
        get_composite_type(lst, spec.id, spec.type, spec.values, 'values', get_enum_items)
    if typ.values then
...
local function register_static_function(_lst, _item)
    return true
end

local register_typedef_item = typed(
    'TypeList, table, table -> boolean, string?',
