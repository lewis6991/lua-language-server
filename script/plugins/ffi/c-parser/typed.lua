...
local function typed_check(val, expected, category, n)
    ...
    if category and n then
        error(
            ('type error: %s %d: expected %s, got %s (%s)'):format(
                category,
                n,
                expected,
                actual,
...
local function typed_function(types, fn)
    ...
    return function(...)
        ...
        if outp == '()' then
            if rets.n ~= 0 then
                error(
                    'wrong number of outputs (given ' .. rets.n .. ' - expects ' .. types .. ')',
                    2
                )
            end
        else
...
function typed.off()
    ...
    typed.typed = function(_, fn)
        return fn
    end
    typed.set_type = function(t, _)
        return t
    end
    typed.table = function(_, t)
        return t
    end
