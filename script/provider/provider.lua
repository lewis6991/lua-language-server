  ...
  function(params)
    ...
    for _, info in ipairs(result) do
      ...
      if rstate then
        ...
        local textEdit =
          converter.textEdit(converter.packRange(rstate, info.start, info.finish), info.text)
...
m.register('workspace/symbol')({
  ...
  function(params)
    local _ <close> =
      progress.create(workspace.getFirstScope().uri, lang.script.WINDOW_PROCESSING_WS_SYMBOL, 0.5)
