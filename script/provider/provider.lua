...
M.register('workspace/didRenameFiles')({
    ...
    function(params)
        ...
        for _, file in ipairs(params.files) do
            ...
            if workspace.isValidLuaUri(oldUri) and workspace.isValidLuaUri(newUri) then
                renames[#renames + 1] = {
                    oldUri = oldUri,
                    newUri = newUri,
                }
        ...
        if targetUri then
            ...
            if targetState then
                if client.getAbility('textDocument.definition.linkSupport') then
                    response[i] = converter.locationLink(
                        targetUri,
...
M.register('textDocument/references')({
    ...
    function(params)
        ...
        for _, info in ipairs(result) do
            ...
            local targetState = files.getState(targetUri)
            if targetState then
                response[#response + 1] = converter.location(
                    targetUri,
                    converter.packRange(targetState, info.target.start, info.target.finish)
                )
            end
    ...
    function(params)
        ...
        for _, info in ipairs(result) do
            ...
            if rstate then
                ...
                local textEdit = converter.textEdit(
                    converter.packRange(rstate, info.start, info.finish),
                    info.text
                )
...
M.register('textDocument/completion')({
    ...
    function(params)
        ...
        if config.get(uri, 'editor.acceptSuggestionOnEnter') ~= 'off' then
            if triggerCharacter == '\n' or triggerCharacter == '{' or triggerCharacter == ',' then
                return
            end
        end
        ...
        for i, res in ipairs(result) do
            local item = {
                ...
                textEdit = res.textEdit
                    and {
                        range = converter.packRange(state, res.textEdit.start, res.textEdit.finish),
            ...
            if res.id then
                if easy and os.clock() - clock < 0.05 then
                    ...
                    if resolved then
                        ...
                        item.documentation = resolved.description
                            and {
...
M.register('textDocument/documentSymbol')({
    ...
    function(params)
        ...
        local function convert(symbol)
            await.delay()
            symbol.range = converter.packRange(state, symbol.range[1], symbol.range[2])
            symbol.selectionRange =
                converter.packRange(state, symbol.selectionRange[1], symbol.selectionRange[2])
            if symbol.name == '' then
                symbol.name = ' '
            end
...
M.register('workspace/symbol')({
    ...
    ---@async
    function(params)
        local _ <close> = progress.create(
            workspace.getFirstScope().uri,
            lang.script.WINDOW_PROCESSING_WS_SYMBOL,
            0.5
        )
        ...
        local function convert(symbol)
            ...
            return {
                ...
                location = converter.location(
                    uri,
                    converter.packRange(state, symbol.source.start, symbol.source.finish)
                ),
            }
        end
        ...
        if not client.getOption('useSemanticByRange') then
            M.register('textDocument/semanticTokens/full')({
                ...
                function(params)
                    ...
                    await.sleep(0.0)
                    local _ <close> =
                        progress.create(uri, lang.script.WINDOW_PROCESSING_SEMANTIC_FULL, 0.5)
...
M.register('$/status/click')({
    ...
    function()
        ...
        local result =
            client.awaitRequestMessage('Info', lang.script.WINDOW_LUA_STATUS_DIAGNOSIS_MSG, {
                titleDiagnostic,
                ...
                label = {
                    {
                        ...
                        location = lstate
                            and converter.location(
                                luri,
                                converter.packRange(lstate, res.source.start, res.source.finish)
                            ),
                    },
                },
...
config.watch(function(_uri, key, _value)
    if key == 'Lua.window.statusBar' or key == '' then
        refreshStatusBar()
    end
end)
